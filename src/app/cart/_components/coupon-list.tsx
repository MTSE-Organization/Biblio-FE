      ...
      <div className='rounded-lg bg-white px-4 py-2'>
        <div className='flex items-center justify-between border-b border-gray-200 pb-2'>
          ...
          <Button
            ...
            className='flex items-center gap-x-2 p-0! text-[#2F80ED] hover:bg-transparent hover:text-[#2f80ed]/80'
          >
            <p>Xem thêm</p>
            <ChevronRight />
          </Button>
        </div>
        {isSelected && couponList.length > 0 && (
          <div className='flex flex-wrap'>
            <div className='mt-2 flex w-full items-center justify-between rounded bg-[#2F80ED33] p-2 text-[#2F80ED]'>
              ...
              <Button
                variant={'ghost'}
                onClick={handleOpenCouponModal}
                className='flex h-3 items-center gap-x-2 p-0! text-[#2F80ED] hover:bg-transparent hover:text-[#2f80ed]/80'
              >
                <ChevronRight />
              </Button>
            </div>
            <Row className='mb-0 flex w-full justify-between'>
              {selectedDiscountCoupon && (
                ...
                >
                  <h3 className='px-4 py-2'>{selectedDiscountCoupon.name}</h3>
                  <X
              ...
              {selectedFreeShipCoupon && (
                ...
                >
                  <h3 className='px-4 py-2'>{selectedFreeShipCoupon.name}</h3>
                  <X
      ...
      <div className='mt-4 rounded-lg bg-white p-4'>
        ...
        {selectedFreeShipCoupon && (
          <div className='mt-4 flex justify-between'>
            ...
            <span>-{formatPrice(selectedFreeShipCoupon.value)}</span>
          </div>
        )}
        {selectedDiscountCoupon && (
          <div className='mt-4 flex justify-between'>
          ...
          <span>
            {formatPrice(
              totalPrice -
                ...
                (totalPrice * +(selectedDiscountCoupon?.value || 0)) / 100
            )}
          </span>
        </div>
        <Button
        ...
        >
          {cartCheckoutMutation.isPending ? <CircleLoading /> : 'Đặt hàng'}
        </Button>
      </div>
      <CouponModal
        onClose={close}
